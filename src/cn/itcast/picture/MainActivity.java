    ...
    private final class MyPictureCallback implements PictureCallback{
		public void onPictureTaken(byte[] data, Camera camera) {
			try {
				...
				outStream.close();
				Log.i("MainActivity", jpgFile.getAbsolutePath());
				Log.i("MainActivity",String.valueOf(jpgFile.exists()));
    ...
    private final class SurfaceCallback implements Callback{
		public void surfaceCreated(SurfaceHolder holder) {
			try{
				...
				Log.i("MainActivity", parameters.flatten());//打印摄像头的各种可设置参数
				parameters.setPreviewSize(800, 480);//设置预览分辨率
				parameters.setPreviewFrameRate(5);
				parameters.setPictureSize(1024,768);//设置拍照的图片分辨率
